...
from nets.quantum_gate import QuantumGateStack, QuantumReplaceBlock
...
class YoloBody(nn.Module):
    def __init__(
        ...
        # 80, 80, 128 => 80, 80, 256
        replace_set = {target.lower() for target in quantum_replace_targets}
        self.rep_conv_1 = (
            QuantumReplaceBlock(transition_channels * 4, transition_channels * 8)
            if "p3" in replace_set
            else conv(transition_channels * 4, transition_channels * 8, 3, 1)
        )
        # 40, 40, 256 => 40, 40, 512
        self.rep_conv_2 = (
            QuantumReplaceBlock(transition_channels * 8, transition_channels * 16)
            if "p4" in replace_set
            else conv(transition_channels * 8, transition_channels * 16, 3, 1)
        )
        # 20, 20, 512 => 20, 20, 1024
        self.rep_conv_3 = (
            QuantumReplaceBlock(transition_channels * 16, transition_channels * 32)
            if "p5" in replace_set
            else conv(transition_channels * 16, transition_channels * 32, 3, 1)
        )
        target_set = {target.lower() for target in quantum_targets}
        self.quantum_p3 = (
            QuantumGateStack(transition_channels * 8, depth=quantum_depth)
            if use_quantum and "p3" in target_set
            else nn.Identity()
        )
        self.quantum_p4 = (
            QuantumGateStack(transition_channels * 16, depth=quantum_depth)
            if use_quantum and "p4" in target_set
            else nn.Identity()
        )
        self.quantum_p5 = (
            QuantumGateStack(transition_channels * 32, depth=quantum_depth)
            if use_quantum and "p5" in target_set
            else nn.Identity()
        )
    ...
    def forward(self, x):
        ...
        P3 = self.quantum_p3(P3)
        P4 = self.quantum_p4(P4)
        P5 = self.quantum_p5(P5)
        #---------------------------------------------------#
        #   第三个特征层
