from __future__ import annotations

import json
import os
import time
from collections import defaultdict
from pathlib import Path

import numpy as np
import torch
from PIL import Image, ImageDraw

from nets.yolo import YoloBody
from utils.utils import cvtColor, get_anchors, get_classes, preprocess_input, resize_image
from utils.utils_bbox import DecodeBox
from yolo import YOLO


ROOT = Path(__file__).resolve().parent
OUTPUT_DIR = ROOT / "quantum_change_origin"
BASELINE_DIR = OUTPUT_DIR / "baseline_preds"
QUANTUM_DIR = OUTPUT_DIR / "quantum_preds"
SUMMARY_PATH = OUTPUT_DIR / "summary.json"
COMPARE_TXT_PATH = OUTPUT_DIR / "compare_quantumnoquantum.txt"
COMPARE_JSON_PATH = OUTPUT_DIR / "compare_quantumnoquantum.json"

MODEL_PATH = ROOT / "model_data" / "yolov7_weights.pth"
COCO_CLASSES_PATH = ROOT / "model_data" / "coco_classes.txt"
ANCHORS_PATH = ROOT / "model_data" / "yolo_anchors.txt"
VAL_ANN_PATH = ROOT / "2007_val.txt"

PHI = "l"
ANCHORS_MASK = [[6, 7, 8], [3, 4, 5], [0, 1, 2]]
INFER_INPUT_SHAPE = [640, 640]
VAL_SUBSET = 30
CONFIDENCE = 0.35
NMS_IOU = 0.3
SEED = 42
QUANTUM_REPLACE_TARGETS = ("p4", "p5")


def set_seed(seed: int) -> None:
    np.random.seed(seed)
    torch.manual_seed(seed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(seed)


def patch_pillow_textsize() -> None:
    if hasattr(ImageDraw.ImageDraw, "textsize"):
        return

    def textsize(self, text, font=None, *args, **kwargs):
        left, top, right, bottom = self.textbbox((0, 0), text, font=font, *args, **kwargs)
        return right - left, bottom - top

    ImageDraw.ImageDraw.textsize = textsize


def patch_torch_from_numpy() -> None:
    original_from_numpy = torch.from_numpy

    def safe_from_numpy(array):
        try:
            return original_from_numpy(array)
        except TypeError as exc:
            if "expected np.ndarray" not in str(exc):
                raise
            dtype = torch.float32 if str(getattr(array, "dtype", "")) == "float32" else None
            if dtype is None:
                return torch.tensor(array.tolist())
            return torch.tensor(array.tolist(), dtype=dtype)

    torch.from_numpy = safe_from_numpy


def load_lines(path: Path, limit: int) -> list[str]:
    lines = [line.strip() for line in path.read_text(encoding="utf-8").splitlines() if line.strip()]
    return [_normalize_annotation_line(line) for line in lines[:limit]]


def _normalize_annotation_line(line: str) -> str:
    parts = line.split()
    if not parts:
        return line
    image_name = Path(parts[0]).name
    image_path = ROOT / "VOCdevkit" / "VOC2007" / "JPEGImages" / image_name
    return " ".join([str(image_path)] + parts[1:])


def _exact_load(model: torch.nn.Module, weights_path: Path, device: torch.device) -> dict[str, int]:
    model_dict = model.state_dict()
    pretrained_dict = torch.load(weights_path, map_location=device)
    matched = {}
    loaded = 0
    skipped = 0
    for key, value in pretrained_dict.items():
        if key in model_dict and tuple(model_dict[key].shape) == tuple(value.shape):
            matched[key] = value
            loaded += 1
        else:
            skipped += 1
    model_dict.update(matched)
    model.load_state_dict(model_dict)
    return {"loaded_keys": loaded, "skipped_keys": skipped}


def build_model(device: torch.device, replace_targets: tuple[str, ...] = ()) -> tuple[YoloBody, dict[str, int]]:
    class_names, _ = get_classes(str(COCO_CLASSES_PATH))
    model = YoloBody(
        ANCHORS_MASK,
        len(class_names),
        PHI,
        pretrained=False,
        use_quantum=False,
        quantum_replace_targets=replace_targets,
    )
    load_info = _exact_load(model, MODEL_PATH, device)
    model.to(device)
    return model, load_info


def build_official_yolo_renderer() -> YOLO:
    patch_pillow_textsize()
    patch_torch_from_numpy()
    return YOLO(
        model_path=str(MODEL_PATH),
        classes_path=str(COCO_CLASSES_PATH),
        cuda=torch.cuda.is_available(),
        confidence=CONFIDENCE,
        nms_iou=NMS_IOU,
    )


def build_renderer_for_model(model: YoloBody) -> YOLO:
    renderer = build_official_yolo_renderer()
    renderer.net = model
    return renderer


def preprocess_pil_image(image: Image.Image, input_shape: list[int]) -> tuple[np.ndarray, torch.Tensor]:
    image = cvtColor(image)
    resized = resize_image(image, (input_shape[1], input_shape[0]), True)
    image_array = np.expand_dims(
        np.transpose(preprocess_input(np.array(resized, dtype="float32")), (2, 0, 1)),
        0,
    )
    return np.array(image), torch.from_numpy(image_array).float()


def run_detector(
    model: YoloBody,
    image_path: Path,
    class_names: list[str],
    bbox_util: DecodeBox,
    device: torch.device,
) -> dict[str, object]:
    image = Image.open(image_path)
    image_shape = np.array(np.shape(image)[0:2])
    _, image_tensor = preprocess_pil_image(image, INFER_INPUT_SHAPE)
    image_tensor = image_tensor.to(device)

    with torch.no_grad():
        if device.type == "cuda":
            torch.cuda.synchronize()
        t0 = time.perf_counter()
        outputs = model(image_tensor)
        if device.type == "cuda":
            torch.cuda.synchronize()
        t1 = time.perf_counter()

        decoded = bbox_util.decode_box(outputs)
        results = bbox_util.non_max_suppression(
            torch.cat(decoded, 1),
            len(class_names),
            INFER_INPUT_SHAPE,
            image_shape,
            True,
            conf_thres=CONFIDENCE,
            nms_thres=NMS_IOU,
        )
        if device.type == "cuda":
            torch.cuda.synchronize()
        t2 = time.perf_counter()

    detections = []
    if results[0] is not None:
        for row in results[0]:
            left, top, right, bottom = map(float, row[:4])
            score = float(row[4] * row[5])
            class_id = int(row[6])
            detections.append(
                {
                    "class_id": class_id,
                    "class_name": class_names[class_id],
                    "score": score,
                    "box": [left, top, right, bottom],
                }
            )
        detections.sort(key=lambda item: item["score"], reverse=True)

    return {
        "image_path": str(image_path),
        "detections": detections,
        "forward_ms": (t1 - t0) * 1000.0,
        "postprocess_ms": (t2 - t1) * 1000.0,
        "total_ms": (t2 - t0) * 1000.0,
    }


def canonicalize_detections(detections: list[dict[str, object]]) -> list[dict[str, object]]:
    normalized = []
    for item in detections:
        normalized.append(
            {
                "class_id": int(item["class_id"]),
                "class_name": str(item["class_name"]),
                "score": round(float(item["score"]), 4),
                "box": [round(float(v), 2) for v in item["box"]],
            }
        )
    return normalized


def box_iou(box1: list[float], box2: list[float]) -> float:
    x1 = max(box1[0], box2[0])
    y1 = max(box1[1], box2[1])
    x2 = min(box1[2], box2[2])
    y2 = min(box1[3], box2[3])
    inter_w = max(0.0, x2 - x1)
    inter_h = max(0.0, y2 - y1)
    inter = inter_w * inter_h
    area1 = max(0.0, box1[2] - box1[0]) * max(0.0, box1[3] - box1[1])
    area2 = max(0.0, box2[2] - box2[0]) * max(0.0, box2[3] - box2[1])
    union = area1 + area2 - inter
    if union <= 0:
        return 0.0
    return inter / union


def center_shift(box1: list[float], box2: list[float]) -> float:
    c1x = (box1[0] + box1[2]) / 2.0
    c1y = (box1[1] + box1[3]) / 2.0
    c2x = (box2[0] + box2[2]) / 2.0
    c2y = (box2[1] + box2[3]) / 2.0
    return float(np.hypot(c1x - c2x, c1y - c2y))


def match_same_class(
    baseline: list[dict[str, object]],
    quantum: list[dict[str, object]],
    iou_threshold: float,
) -> tuple[list[tuple[dict[str, object], dict[str, object], float]], list[dict[str, object]], list[dict[str, object]]]:
    pairs = []
    for bi, bdet in enumerate(baseline):
        for qi, qdet in enumerate(quantum):
            iou = box_iou(bdet["box"], qdet["box"])
            if iou >= iou_threshold:
                pairs.append((iou, bi, qi))
    pairs.sort(reverse=True)

    used_b = set()
    used_q = set()
    matched = []
    for iou, bi, qi in pairs:
        if bi in used_b or qi in used_q:
            continue
        used_b.add(bi)
        used_q.add(qi)
        matched.append((baseline[bi], quantum[qi], iou))

    missing = [baseline[i] for i in range(len(baseline)) if i not in used_b]
    extra = [quantum[i] for i in range(len(quantum)) if i not in used_q]
    return matched, missing, extra


def describe_detection(det: dict[str, object]) -> str:
    box = det["box"]
    return (
        f"{det['class_name']}(score={det['score']:.4f},"
        f" box=[{box[0]:.1f},{box[1]:.1f},{box[2]:.1f},{box[3]:.1f}])"
    )


def compare_detections(
    baseline: list[dict[str, object]],
    quantum: list[dict[str, object]],
    iou_threshold: float = 0.5,
    score_threshold: float = 0.0001,
    shift_threshold: float = 0.05,
) -> list[str]:
    grouped_baseline: dict[str, list[dict[str, object]]] = defaultdict(list)
    grouped_quantum: dict[str, list[dict[str, object]]] = defaultdict(list)
    for det in baseline:
        grouped_baseline[str(det["class_name"])].append(det)
    for det in quantum:
        grouped_quantum[str(det["class_name"])].append(det)

    messages: list[str] = []
    class_names = sorted(set(grouped_baseline) | set(grouped_quantum))
    for class_name in class_names:
        matched, missing, extra = match_same_class(
            grouped_baseline.get(class_name, []),
            grouped_quantum.get(class_name, []),
            iou_threshold=iou_threshold,
        )
        for det in missing:
            messages.append(f"{describe_detection(det)} 缺失")
        for det in extra:
            messages.append(f"{describe_detection(det)} 多余")
        for bdet, qdet, iou in matched:
            delta = float(qdet["score"]) - float(bdet["score"])
            if abs(delta) >= score_threshold:
                messages.append(f"{class_name} 置信度变化({delta:+.4f})")
            shift = center_shift(bdet["box"], qdet["box"])
            if shift >= shift_threshold:
                messages.append(f"{class_name} 偏移(IoU={iou:.3f}, 中心偏移={shift:.2f}px)")

    if not messages:
        messages.append("无明显差异")
    return messages


def summarize_group(name: str, records: list[dict[str, object]]) -> dict[str, object]:
    mean_forward = sum(float(record["forward_ms"]) for record in records) / len(records)
    mean_post = sum(float(record["postprocess_ms"]) for record in records) / len(records)
    mean_total = sum(float(record["total_ms"]) for record in records) / len(records)
    mean_detections = sum(len(record["detections"]) for record in records) / len(records)
    return {
        "名称": name,
        "图像数量": len(records),
        "平均前向耗时毫秒": mean_forward,
        "平均后处理耗时毫秒": mean_post,
        "平均总耗时毫秒": mean_total,
        "平均检测框数量": mean_detections,
        "样例结果": [
            {
                "图像路径": record["image_path"],
                "前向耗时毫秒": record["forward_ms"],
                "后处理耗时毫秒": record["postprocess_ms"],
                "总耗时毫秒": record["total_ms"],
                "检测结果前五项": canonicalize_detections(record["detections"][:5]),
            }
            for record in records[:5]
        ],
    }


def clear_output_dir(path: Path) -> None:
    path.mkdir(parents=True, exist_ok=True)
    for file in path.glob("*.png"):
        file.unlink()


def write_compare_files(
    image_ids: list[str],
    baseline_records: list[dict[str, object]],
    quantum_records: list[dict[str, object]],
) -> dict[str, object]:
    txt_lines: list[str] = []
    json_rows: list[dict[str, object]] = []
    identical = 0
    changed = 0

    for image_id, baseline, quantum in zip(image_ids, baseline_records, quantum_records):
        diff_messages = compare_detections(baseline["detections"], quantum["detections"])
        if diff_messages == ["无明显差异"]:
            identical += 1
        else:
            changed += 1
        txt_lines.append(f"pngid:{image_id}")
        txt_lines.append(f"quantum的差异：{'；'.join(diff_messages)}")
        json_rows.append(
            {
                "pngid": image_id,
                "quantum的差异": diff_messages,
                "baseline_total_ms": baseline["total_ms"],
                "quantum_total_ms": quantum["total_ms"],
            }
        )

    COMPARE_TXT_PATH.write_text("\n".join(txt_lines) + "\n", encoding="utf-8-sig")
    COMPARE_JSON_PATH.write_text(
        json.dumps(json_rows, ensure_ascii=False, indent=2),
        encoding="utf-8-sig",
    )

    return {
        "检测结果无明显差异的图像数量": identical,
        "检测结果发生变化的图像数量": changed,
        "文本对比文件": str(COMPARE_TXT_PATH),
        "JSON对比文件": str(COMPARE_JSON_PATH),
    }


def run_comparison(device: torch.device) -> dict[str, object]:
    class_names, num_classes = get_classes(str(COCO_CLASSES_PATH))
    anchors, _ = get_anchors(str(ANCHORS_PATH))
    bbox_util = DecodeBox(anchors, num_classes, (INFER_INPUT_SHAPE[0], INFER_INPUT_SHAPE[1]), ANCHORS_MASK)

    baseline_model, baseline_load = build_model(device, replace_targets=())
    baseline_model = baseline_model.fuse().eval()
    baseline_renderer = build_official_yolo_renderer()

    quantum_model, quantum_load = build_model(device, replace_targets=QUANTUM_REPLACE_TARGETS)
    quantum_model = quantum_model.eval()
    quantum_renderer = build_renderer_for_model(quantum_model)

    lines = load_lines(VAL_ANN_PATH, VAL_SUBSET)
    image_paths = [Path(line.split()[0]) for line in lines]
    image_ids = [path.stem for path in image_paths]

    clear_output_dir(BASELINE_DIR)
    clear_output_dir(QUANTUM_DIR)

    baseline_records = []
    quantum_records = []
    for image_path in image_paths:
        baseline = run_detector(baseline_model, image_path, class_names, bbox_util, device)
        quantum = run_detector(quantum_model, image_path, class_names, bbox_util, device)
        baseline_records.append(baseline)
        quantum_records.append(quantum)

        stem = image_path.stem
        rendered_baseline = baseline_renderer.detect_image(Image.open(image_path))
        rendered_baseline.save(BASELINE_DIR / f"{stem}_baseline.png", quality=95, subsampling=0)
        rendered_quantum = quantum_renderer.detect_image(Image.open(image_path))
        rendered_quantum.save(QUANTUM_DIR / f"{stem}_quantum.png", quality=95, subsampling=0)

    compare_summary = write_compare_files(image_ids, baseline_records, quantum_records)
    baseline_summary = summarize_group("原始预训练YOLOv7", baseline_records)
    quantum_summary = summarize_group("替换经典结构后的无训练量子YOLOv7", quantum_records)

    return {
        "基线模型": baseline_summary,
        "量子替换模型": quantum_summary,
        "基线权重加载": baseline_load,
        "量子替换权重加载": quantum_load,
        "差异统计": compare_summary,
        "总耗时差值毫秒_量子减基线": quantum_summary["平均总耗时毫秒"] - baseline_summary["平均总耗时毫秒"],
        "量子替换模型是否更快": quantum_summary["平均总耗时毫秒"] < baseline_summary["平均总耗时毫秒"],
    }


def to_serializable(obj):
    if isinstance(obj, np.floating):
        return float(obj)
    if isinstance(obj, np.integer):
        return int(obj)
    if isinstance(obj, Path):
        return str(obj)
    raise TypeError(f"Object of type {type(obj)!r} is not JSON serializable")


def main() -> None:
    os.environ.setdefault("PYTHONNOUSERSITE", "1")
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    set_seed(SEED)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"运行量子替换原结构实验，device={device}")

    comparison_summary = run_comparison(device)
    summary = {
        "实验名称": "量子替换原经典结构的YOLOv7前向对照",
        "设备": str(device),
        "随机种子": SEED,
        "推理输入尺寸": INFER_INPUT_SHAPE,
        "置信度阈值": CONFIDENCE,
        "NMS阈值": NMS_IOU,
        "量子替换配置": {
            "替换位置": list(QUANTUM_REPLACE_TARGETS),
            "每个替换块的theta个数": 9,
            "是否训练": False,
            "替换策略": "用无训练量子替换块替换P4/P5进检测头前的rep_conv结构",
        },
        "说明": [
            "基线模型使用原始COCO预训练YOLOv7。",
            "量子模型没有在原经典层后面再加门控，而是直接替换掉P4和P5的rep_conv结构。",
            "替换块内部使用3个qubit和9个theta，不做训练，只做前向推理对照。",
            "由于替换块没有继承被替换经典层的预训练卷积权重，因此结果差异通常会比插入式量子门控更明显。",
        ],
        "对照结果": comparison_summary,
        "输出文件": {
            "输出目录": str(OUTPUT_DIR),
            "基线图片目录": str(BASELINE_DIR),
            "量子图片目录": str(QUANTUM_DIR),
            "文本对比文件": str(COMPARE_TXT_PATH),
            "JSON对比文件": str(COMPARE_JSON_PATH),
        },
    }

    SUMMARY_PATH.write_text(
        json.dumps(summary, ensure_ascii=False, indent=2, default=to_serializable),
        encoding="utf-8",
    )
    print(f"Summary written to: {SUMMARY_PATH}")


if __name__ == "__main__":
    main()
